...
Статус: закрыты вехи 1–8, веха 9 наполовину. Есть окно, есть раздача с автообновлением,
есть страница загрузки. Главное, чего не было в исходном плане: **программа сама водит
ИИ-помощника пользователя** и разбирает незнакомую игру по нажатию кнопки. Этот файл —
рабочий документ, обновляется по ходу.
...
|---|---|---|
| **1** | `core/paths/`, `core/steam.py`, сканер Wine-префиксов, каркас репо, CI-матрица | **готово** |
| **2** | Ядро плагинов, `pipeline`, `ops/`, 3 плагина вручную, round-trip тесты | **готово** |
| **3** | Классификатор риска, `risk_db.json`, мастер Steam Cloud | **готово** |
| **4** | CLI: `scan` / `parse` / `set` / `backup` | **готово** |
| **5** | GUI на Tauri | **готово**, кроме подписи и нотаризации: нужны платные сертификаты |
| **6** | Агент discovery (лестница декодеров → LLM последним) | **готово** |
| **7** | Детект чексумм | **готово** |
| **8** | Репозиторий плагинов: экспорт/импорт, версии, установка из GitHub | **готово** |
| 9 | Выдача предметов в инвентарь | **наполовину**: чтение, правка количества и выдача расходников есть; оружие и броня во FromSoftware — нет |
| **10** | Раздача: релиз по тегу, автообновление, страница загрузки, приём плагинов от пользователей | **готово** |
| **11** | ИИ внутри продукта: MCP-сервер и кнопка «Разобрать эту игру» | **готово** |

---

## 2г. Веха 11 — ИИ перестал быть отдельной покупкой (2026-08-08)

Веха 6 сделала discovery с моделью, но за ключ API и по токенам. Это стена: обычный
человек с подпиской на Claude Code не мог воспользоваться главной функцией продукта.

Решения, принятые по ходу:

1. **Два режима, и приоритет у второго.** `savesmith mcp` отдаёт инструменты чужому
   агенту — это для тех, кто и так живёт в терминале. А `agent/assistant.py` наоборот:
   программа сама находит установленного помощника, сама пишет ему задание и ведёт
   разбор. Человек нажимает кнопку и смотрит на строки прогресса. Писать промпт — наша
   работа, метод мы знаем лучше него.
2. **Модель не пишет код.** Она собирает конвейер из наших проверенных операций.
   Исполнять нечего — значит, песочница не нужна вовсе, и это строго безопаснее вехи 6.
3. **Смотреть и предлагать можно, писать в сейв — нельзя.** Проверяется тестом,
   перечисляющим инструменты поимённо. Обещания промпта продублированы кодом:
   `propose_plugin` опускает confidence до experimental и не даёт тиру быть safe.
4. **Числа спрашиваются у человека формой, а не свободным текстом.** Поле называется,
   только если найдено по числу с экрана или названо самой игрой; иначе плагин
   подпишет счётчик голода «здоровьем», и это не поймает ни один гейт.

## 2б. Изменение политики риска (решение от 2026-08-06)

Исходная спецификация требовала **отказываться** работать с играми тира `BLOCKED`
(FromSoftware, EasyAntiCheat, общая прогрессия сингла и онлайна). Решение изменено:

**`BLOCKED` перестаёт быть отказом и становится экраном подтверждения.** Пользователь видит
крупно, чем именно рискует, и подтверждает явным действием. Основная аудитория продукта —
как раз игры вроде Elden Ring, где нужно выдать золото или предметы.

Что из этого следует:

1. **Детект анти-чита становится главным механизмом, а не поводом для отказа.** Сканируем папку
   игры на `EasyAntiCheat*`, `BattlEye*`, `*_EAC*`, `*.sys`. Анти-чит не найден (пиратская сборка,
   кряк, или игра его не несёт) — тир падает, работаем с обычным предупреждением. Найден —
   экран с объяснением последствий и подтверждением.
2. **Веха 7 (пересчёт контрольных сумм) переезжает вперёд.** Сейв Elden Ring — это контейнер
   BND4: десять слотов, каждый под AES-CBC, у каждого MD5 в заголовке. Без пересчёта сумм игра
   такой сейв отвергает или «чинит». То есть флагманский кейс продукта физически не работает,
   пока веха 7 не сделана. Нужен ещё и разбор BND4 отдельной операцией.
3. **Тир `SAFE` по-прежнему не выдаётся по умолчанию.** Неизвестная игра остаётся `CAUTION`.

Что **не** меняется: SaveSmith не отключает и не патчит анти-чит, не трогает `.exe`, `.dll`,
ассеты и память процесса. Это тот самый архитектурный принцип, который выводит продукт из
категории читов и трейнеров; отключение защиты — другой продукт, и смешивать их нельзя.
Приложение может показать, что анти-чит найден и чем это грозит, — не более.

## 2в. Адаптер к модели (решение от 2026-08-07)

Последний недостающий кусок вехи 6 — `agent/writer.py`. Четыре решения.

1. **Anthropic SDK, а не `claude-agent-sdk`.** Спецификация называла agent-sdk, но цикл
   «предложи кодек → выполни в песочнице → верни ошибку → повтори» у нас уже написан своей
   машиной состояний, с бюджетом и гейтом. Agent-sdk принёс бы вторую петлю поверх нашей и
   свои встроенные инструменты работы с файлами — ровно то, чего этому шагу иметь не нужно.
   Берётся обычный `anthropic`, модель `claude-opus-5`, adaptive thinking, стриминг.
2. **Гейт — round-trip, а не «код запустился».** Кодек принимается, только если
   `encode(decode(файл))` совпадает с файлом побайтово. К исходнику модели дописывается
   `_savesmith_check`, который это и проверяет внутри песочницы и сообщает смещение первого
   расхождения. Кодек, который что-то декодирует, но не собирает файл обратно, — это не
   кодек, а способ испортить сейв.
3. **Бюджет считается до запроса, а не после.** Каждая попытка оценивается по худшему случаю
   (промпт через `count_tokens` плюс весь лимит вывода по цене модели) и сравнивается с
   остатком. Поэтому лимит `--budget` — это потолок, а не пожелание.
4. **`anthropic` не в зависимостях.** Ставится отдельно и нужен только для `--model`.
   Импорт ленивый; без пакета, без ключа или без бюджета шаг отказывается и пишет, что
   именно из трёх, а discovery дорабатывает остальные шаги и печатает отчёт.

Обратно к модели уходит только текст исключения, смещение и имена полей, которые вернул
кодек. Файл не уходит никуда: модель пишет код, код работает на машине пользователя.

## 2а. Что изменилось по ходу вехи 1

Три расхождения с планом, все осознанные.

1. **`_macos.py` не появился.** Планировался симметрично `_windows.py`, но на macOS нет
   ни одного нативного вызова: все токены — это склейка путей от `home()`. Таблица токенов
   обеих платформ живёт в `_resolver.py` двумя явно разделёнными функциями. Появится, когда
   в вехе 2 понадобится `plistlib` для Unity PlayerPrefs.
2. **Добавлена `paths.locate()`** — регистронезависимый поиск *литерального* пути. Понадобилась
   в `steam.py`: библиотека в `D:\Games [SSD]` содержит glob-символы, и `resolve()` принял бы их
   за шаблон и молча ничего не нашёл.
3. **Нативные тесты помечаются `native_windows` / `native_macos` и пропускают себя сами**,
   вместо отдельного прогона `pytest -m native` из плана. CI гоняет одну команду на всех
   раннерах, забыть про второй шаг невозможно.

Плюс два бага, пойманных собственными сторожами до того, как их увидел бы пользователь:
тест-страж платформенных ветвлений сработал на docstring (переписан на `tokenize`), а
определение вида бутылки искало подстроку в любом компоненте пути — папка «My Whisky Notes»
считалась бы бутылкой Whisky.
